...
for( i in 1:length(label_cn) ){
    ...
    grid_radial <- expand.grid(
        sigma = c(0,0.01, 0.025, 0.03, 0.04, 
                0.05, 0.07, 0.09, 0.1, 
                0.25, 0.5, 0.75,0.9),
        C = c(0,0.01, 0.1, 0.25, 0.5,
        1, 1.5, 2,5)
    )
    train_ctrl <- trainControl(method = 'repeatedcv', number=10, 
                    repeats = 4, verboseIter=T, search='random')
    cl <- makePSOCKcluster( cores )
    registerDoParallel(cl) 
    train_svm[[ label_cn[i] ]] <- train(x = train_x, y = train_y, method = 'svmRadial',
...
radial_svm <- list( train_svm = train_svm, train_pred = train_pred, test_pred =test_pred)
save(radial_svm, file="radial_svm.Rdata")
