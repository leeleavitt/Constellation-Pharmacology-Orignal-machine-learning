...
setwd("Z:/Lee Leavitt/ML experiments/Final Experiment")

#Load the data
feature_fname <- list.files(pattern = "features")
feature_space <- read.csv(feature_fname)

label_fname <- list.files(pattern = "labels")
...
for( i in 1:length(label_cn) ){
    ...
    #SVM
    grid <- expand.grid(C = c(1e-2, 1e-1, 1, 1e2, 1e3, 1e4))

    train_ctrl <- trainControl(method = 'repeatedcv', number=3, 
